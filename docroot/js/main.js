function ServeJavaResource(){
	AUI().use('event',
			'node',
			'aui-io-request',
			function(A){
		
		    var el_1 = A.one('#_1_WAR_chatportlet_question');
		    if (el_1){
		    	
		    		   var myQuestion = el_1.val();
		    		   var url = Liferay.PortletURL.createResourceURL();
		  	  	       url.setPortletId('1_WAR_chatportlet');
		  	  	       url.setResourceId('serveAnswer');
		  	  	       
		  	  	       var finalurl = 'http://localhost:8080' + url + "&question=" + myQuestion; 
		  	  	       A.io.request(finalurl,
		                      {
		                         method:'GET',
		                         //dataType: 'text',
		                         //data: {
						         ...
						         on: {
					                 success: function() {
					                  var out = this.get('responseData');
					                  
					                  var answerBox = A.one('#_1_WAR_chatportlet_answer');
					                  var outtoText = JSON.stringify(out, null, 4)
					                  answerBox.set('value', outtoText);
...
client.listen('call', function(evt) { 
	call = evt.call;
	//Revicieving party will initiate this call.
	...
	function(A) {
		...
		Panel.prototype = {
			...
			_createPanel: function(fromMarkup) {
				...
				panel.all('.respoke-bar').on(
						'click',
						function(event) {
							var target = event.currentTarget;
							var endpoint = client.getEndpoint({"id" : userIdforRespoke});
							if (target.hasClass('talk')) {
								//alert(userIdforRespoke);
								
							    call = endpoint.startAudioCall();
							}
							else if (target.hasClass('screenshare')) {
					...
					var html = '<li class="user user_' + instance._panelId + '" panelId="' + instance._panelId + '">' +
									...
									'<div class="chat-panel">' +
										'<div class="panel-window">' +
											...
											'</div>' +
											'<div class="panel-input">' +
												'<div class="respoke-bar video"></div>' +
												'<div class="respoke-bar talk"></div>' +
												'<div class="respoke-bar screenshare"></div>' +
											'</div>' +
										    '<div class="panel-output" id="remoteVideoSource"></div>' +
										    '<div class="panel-output" id="localVideoSource"></div>'+
										'</div>' +
									'</div>' +
								'</li>';
